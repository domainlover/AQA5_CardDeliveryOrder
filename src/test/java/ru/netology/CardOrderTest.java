...
public class CardOrderTest {
    ...
    @Test
    void shouldAcceptOrderWithRareName() {
        CardDelivery user = DataGenerator.Generate.generateUserData("ru");
        $("[data-test-id=city] input").setValue(user.getCity());
        $("[data-test-id=date] [value]").doubleClick().sendKeys(Keys.DELETE);
        $("[data-test-id=date] [value]").setValue(generateDeliveryDate(4));
        $("[name='name']").setValue(DataGenerator.Generate.generateRareUserName());
        $("[name='phone']").setValue(user.getPhone());
        $("[data-test-id=agreement]").click();
        $("[class='button__text']").click();
        $("[data-test-id=success-notification]").waitUntil(Condition.visible, 3000).shouldHave(exactText("Успешно! Встреча успешно запланирована на " + generateDeliveryDate(4)));
    }
